...
*/
    ...
    float PID::refresh(const float &feedback_input)
    {
        ...
        {
            last_output = Kp*error + Ki*integral + Kd*(error-last_error);       //PID
//            last_output = Kp*error + Kd*(error-last_error);                 //PD
//            std::cout << last_output <<std::endl;

            if(abs(last_output) < deadzone and last_output != 0)
                last_output = last_output>0?deadzone:-deadzone;
    ...
    void PID::setDeadZone(const float &deadzone)
    {
